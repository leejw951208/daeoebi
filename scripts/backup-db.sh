...
# cron 예시:  0 3 * * *  cd /opt/secrets-manager && ./scripts/backup-db.sh >> /var/log/sm-backup.log 2>&1
set -eu
...
FILE="secrets-manager-${STAMP}.sql.gz"
TMP="$(mktemp -t sm-backup.XXXXXX)"
trap 'rm -f "$TMP"' EXIT

# 컨테이너 안에서 pg_dump 실행 → gzip 압축 → 임시파일.
docker exec -i secrets-manager-postgres \
