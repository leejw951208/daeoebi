...
> 한국 금융·쇼핑 자격증명을 도메인 메타데이터(계좌번호·카드번호·OTP 등)와 함께 한 로컬 vault 에 보관하는 Secrets Manager 기능.
...
Secrets Manager는 이미 "내 재무생활"을 한 곳에서 다루는 로컬 단일 사용자 도구이므로, 같은 도메인 관점으로 자격증명을 합류시킨다. 이로써 추후 "이 카드의 결제 계정은 이 계좌" 같은 도메인 조인을 같은 SQLite 안에서 만들 수 있다.
...
- 저장소. 기존 SQLite(`apps/api/data/secrets-manager.db`) 의 새 테이블.
...
- Bitwarden 어댑터 + Secrets Manager 메타만 저장. 두 리뷰어가 추천했으나 도메인 메타 일등 시민이라는 Builder 모드 방향을 유지하기 위해 기각.
