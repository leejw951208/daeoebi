...
| T004 | P2 | `apps/api`에 Prisma 설치, `prisma/schema.prisma`에 SQLite 데이터소스(`file:./data/secrets-manager.db`) 설정, `RecurringExpense`, `ExpenseOccurrence` 모델과 enum(`Recurrence`, `OccurrenceStatus`) 정의, 첫 마이그레이션 생성·적용, `PrismaModule`/`PrismaService` 작성. | TODO | T003 | 1h | |
